...
<!-- 多选：<UserSelect v-model:value="formData.userIds" :maxNum="10" />  formData.userIds 为字符串逗号拼接 -->
<template>
...
const multiple = computed(() => props.maxNum !== 1)
  ...
  props: {
    rowClick ({ key, record }) {
      if (!selectUserIds.value.includes(key)) {
        if (props.maxNum >= 1 && selectUsers.value.length >= props.maxNum) {
