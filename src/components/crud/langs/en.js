/**
 * 英文
 */
export default {
  // 框架
  frame: {
    themeSetting: 'Theme Settings',
    themeMode: 'Light/Dark',
    setting: 'Settings',
    theme: 'Theme',
    themeGreen: 'Minty Fresh',
    themeGolden: 'Golden Beach ',
    themeBlue: 'Coastal Breeze',
    themePink: 'Pink Gentle',
    themeDiy: 'Customization',
    message: 'Message',
    selectTheme: 'Select Theme',
    colorPrimary: 'Primary Color',
    colorError: 'Error Color',
    colorWarning: 'Warning Color',
    colorSuccess: 'Success Color',
    colorInfo: 'Link Color',
    componentSize: 'Component Size',
    menuLayout: 'Menu Layout',
    leftMenuWidth: 'Left Menu Width',
    topHeaderHeight: 'Top Header Height',
    autoOpenChildMenu: 'Automatically open submenus',
    autoOpenChildMenuTooltip: 'Click the top menu to automatically open the last opened submenu or the first submenu.',
    formLayout: 'Form Layout',
    horizontal: 'horizontal',
    vertical: 'vertical',
    inline: 'inline',
    small: 'Small',
    middle: 'Medium',
    large: 'Large',
    themeMode: 'Theme Mode',
    modeLight: 'Light',
    modeDark: 'Dark',
    modeAuto: 'System',
    modeAnimate: 'Mode Animation',
    modeAnimateFade: 'Fade In/Out',
    modeAnimateCircle: 'Circle Clip',
    modeAnimateLine: 'Line Clip',
    radiusLayout: 'Rounded Layout',
    layoutDivide: 'Layout Divide',
    layoutDivideNone: 'None',
    layoutDivideLine: 'Line',
    layoutDivideShadow: 'Shadow',
    layoutDivideRound: 'Round',
    useFooter: 'Show Footer',
    tabs: 'Tabs',
    useTabs: 'Use Tabs Page',
    tabAnimate: 'Tab Page Animation',
    tabAnimateNone: 'None',
    tabAnimateSlideRight: 'Slide Right',
    tabAnimateFade: 'Fade',
    tabAnimateScale: 'Scale',
    tabType: 'Tab Style',
    tabTypeTab: 'Tab',
    tabTypeCard: 'Card',
    tabTypeLine: 'Line',
    useTabIcon: 'Show Icon',
    useTabClose: 'Show Close Icon',
    useDynamicPageTitle: 'Dynamic Page Title',
    lang: 'Language',
    selectLang: 'Select Language',
    animate: 'Animation',
    other: 'Other',
    showWeather: 'Show Weather',
    personalCenter: 'Personal Center',
    logout: 'Log Out',
    notLogin: 'Not Logged In',
    fullscreen: 'Fullscreen',
    exitFullscreen: 'Exit Fullscreen',
    closeTab: 'Close Tab',
    closeLeft: 'Close Left',
    closeRight: 'Close Right',
    closeOther: 'Close Others',
    closeAll: 'Close All',
    top: 'Top',
    left: 'Left',
    topLeft: 'TopLeft',
    useTableBorder: 'Use Table Border',
    useThinLine: 'Use Thin Border',
    useThinLineExtra: 'Requires support from monitor and browser',
    useBreadcrumbs: 'Use Breadcrumbs',
    useWatermark: 'Use Watermark',
    search: 'Search',
    searchRecord: 'Search History',
    noSearchResult: 'No results found',
    firstMenuMode: 'Top Menu Style',
    firstMenuModeLine: 'Line',
    firstMenuModeCard: 'Card',
    firstMenuAlign: 'Top Menu Position',
    firstMenuAlignLeft: 'Left',
    firstMenuAlignCenter: 'Center',
    firstMenuAlignRight: 'Right',
    usePrimaryColor: 'Use Primary Color',
    refreshTab: 'Reload',
    openInNewWindow: 'Open in new Tab',
    useTabFavorite: 'Use Favorites',
    remove: 'Remove',
    favorite: 'Favorite',
    removeFavorite: 'Cancel Favorite',
  },
  // crud
  crud: {
    add: 'Add',
    batchDelete: 'Batch Delete',
    export: 'Export',
    back: 'Back',
    search: 'Search',
    reset: 'Reset',
    rememberSearch: 'Remember Search',
    notLog: 'No History',
    pleaseEnterRemark: 'Please enter remarks',
    pleaseEnterSearchParams: 'Please fill in search criteria first',
    submit: 'Submit',
    cancel: 'Cancel',
    notNull: '{label} cannot be empty',
    someRowNotNull: 'Row {rowNum} [{label}] cannot be empty',
    clickToDownload: 'Click to download',
    importTemplate: 'Import Template',
    dragFileToUpload: 'Drag file here or click to upload',
    pleaseUploadExcel: 'Please upload .xls, .xlsx files',
    sameDataIsReplace: 'Overwrite duplicate records?',
    import: 'Import',
    importSuccess: 'Import successful',
    yes: 'Yes',
    no: 'No',
    edit: 'Edit',
    detail: 'Detail',
    close: 'Close',
    delete: 'Delete',
    deleteSuccess: 'Deleted successfully',
    disabled: 'Disabled',
    enabled: 'Enabled',
    toggleSuccess: 'Status toggled successfully',
    warmTips: 'Tips',
    confirmDeleteBySelected: 'Are you sure you want to delete selected items ({value} items)?',
    addSuccess: 'Added successfully',
    updateSuccess: 'Updated successfully',
    pleaseEnter: 'Please enter {text}',
    pleaseSelect: 'Please select {text}',
    pleaseSelectStart: 'Select start {text}',
    pleaseSelectEnd: 'Select end {text}',
    pleaseEnterOrSelect: 'Please enter or select {text}',
    start: 'Start',
    end: 'End',
    all: 'All',
    refreshSuccess: 'Refreshed successfully',
    refreshData: 'Refresh Data',
    confirmDoIt: 'Are you sure to perform {text} operation on this record?',
    this: 'This',
    more: 'More',
    count: 'Total',
    pleaseEnterKeywordToSearch: 'Please enter keyword to search',
    totalPage: 'total {value} rows',
  }
}