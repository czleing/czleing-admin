/**
 * 中文简体
 */
export default {
  // 框架
  frame: {
    themeSetting: '主题设置',
    themeMode: '明暗模式',
    setting: '设置',
    theme: '主题',
    themeGreen: '薄荷清新',
    themeGolden: '金色沙滩',
    themeBlue: '海岸微风',
    themePink: '轻胭落砚',
    themeDiy: '自定义',
    message: '通知消息',
    selectTheme: '选择主题',
    colorPrimary: '主题色',
    colorError: '危险色',
    colorWarning: '警告色',
    colorSuccess: '成功色',
    colorInfo: '超链接文字色',
    componentSize: '组件大小',
    layout: '布局',
    menu: '菜单',
    menuLayout: '菜单布局',
    leftMenuWidth: '左侧菜单宽度',
    topHeaderHeight: '顶部高度',
    autoOpenChildMenu: '自动打开子菜单',
    autoOpenChildMenuTooltip: '点击顶部菜单时自动打开最后一次打开的子菜单或第一个子菜单',
    formLayout: '表单布局',
    horizontal: 'horizontal',
    vertical: 'vertical',
    inline: 'inline',
    small: '小',
    middle: '中',
    large: '大',
    themeMode: '明暗模式',
    modeLight: '浅色',
    modeDark: '深色',
    modeAuto: '跟随系统',
    modeAnimate: '明暗动画',
    modeAnimateFade: '淡入淡出',
    modeAnimateCircle: '圆形扩散',
    modeAnimateLine: '线性扩散',
    radiusLayout: '圆润布局',
    layoutDivide: '布局边界',
    layoutDivideNone: '无',
    layoutDivideLine: '线条',
    layoutDivideShadow: '阴影',
    layoutDivideRound: '间隔',
    useFooter: '显示页脚',
    tabs: '标签栏',
    useTabs: '使用标签页',
    tabAnimate: '标签页动画',
    tabAnimateNone: '无',
    tabAnimateSlideRight: '右滑',
    tabAnimateFade: '淡入',
    tabAnimateScale: '缩放',
    tabType: '标签样式',
    tabTypeTab: '页签',
    tabTypeCard: '卡片',
    tabTypeLine: '线条',
    useTabIcon: '显示图标',
    useTabClose: '显示关闭按钮',
    useDynamicPageTitle: '动态网页标题',
    lang: '语言',
    selectLang: '选择语言',
    animate: '动画',
    other: '其他',
    showWeather: '展示天气',
    personalCenter: '个人中心',
    logout: '退出系统',
    notLogin: '未登录',
    fullscreen: '全屏',
    exitFullscreen: '退出全屏',
    closeTab: '关闭标签',
    closeLeft: '关闭左侧',
    closeRight: '关闭右侧',
    closeOther: '关闭其他',
    closeAll: '关闭全部',
    top: '顶部',
    left: '左侧',
    topLeft: '混合',
    useTableBorder: '表格竖向边框',
    useThinLine: '使用细线边框',
    useThinLineExtra: '需显示器、浏览器支持',
    useBreadcrumbs: '使用面包屑',
    useWatermark: '使用水印',
    useWanSplit: '数字万分位',
    search: '搜索',
    searchRecord: '搜索历史',
    noSearchResult: '未搜索到相关结果',
    firstMenuMode: '顶部菜单样式',
    firstMenuModeLine: '线条',
    firstMenuModeCard: '卡片',
    firstMenuAlign: '顶部菜单位置',
    firstMenuAlignLeft: '居左',
    firstMenuAlignCenter: '居中',
    firstMenuAlignRight: '居右',
    usePrimaryColor: '使用主题色',
    refreshTab: '重新载入',
    openInNewWindow: '在新窗口打开',
    useTabFavorite: '使用收藏',
    remove: '移除',
    favorite: '收藏',
    removeFavorite: '取消收藏',
    useBg: '使用背景图',
    bgImage: '背景图',
    bgBlur: '背景模糊度',
    bgOpacity: '背景不透明度',
    bgDiy: '自定义',
    restoreSettings: '恢复默认设置',
    restoreSettingsConfirm: '确认恢复默认设置吗？',
  },
  // crud
  crud: {
    add: '新增',
    batchDelete: '批量删除',
    export: '导出',
    back: '返回',
    search: '查询',
    reset: '重置',
    rememberSearch: '记住查询',
    notLog: '暂无历史记录',
    pleaseEnterRemark: '请输入备注',
    pleaseEnterSearchParams: '请先填写查询条件',
    submit: '提交',
    cancel: '取消',
    notNull: '{label}不能为空',
    someRowNotNull: '第 {rowNum} 行[{label}]不能为空',
    clickToDownload: '点击此处下载',
    importTemplate: '导入模板',
    dragFileToUpload: '将文件拖到此处，或点击上传',
    pleaseUploadExcel: '请上传 .xls, .xlsx 文件',
    sameDataIsReplace: '重复记录是否覆盖更新',
    import: '导入',
    importSuccess: '导入成功',
    yes: '是',
    no: '否',
    edit: '编辑',
    detail: '详情',
    close: '关闭',
    delete: '删除',
    deleteSuccess: '删除成功',
    disabled: '禁用',
    enabled: '启用',
    toggleSuccess: '启/禁用成功',
    warmTips: '温馨提示',
    confirmDeleteBySelected: '确定要删除选中项(共 {value} 项)吗？',
    addSuccess: '新增成功',
    updateSuccess: '修改成功',
    pleaseEnter: '请输入{text}',
    pleaseSelect: '请选择{text}',
    pleaseSelectStart: '请选择{text}起',
    pleaseSelectEnd: '请选择{text}止',
    pleaseEnterOrSelect: '请输入或选择{text}',
    start: '起',
    end: '止',
    all: '全部',
    refreshSuccess: '刷新成功',
    refreshData: '刷新数据',
    confirmDoIt: '确定对该条数据执行{text}操作吗？',
    this: '此',
    more: '更多',
    count: '合计',
    pleaseEnterKeywordToSearch: '请输入关键字搜索',
    totalPage: '共 {value} 条',
  },
}