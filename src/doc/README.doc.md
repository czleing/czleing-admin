...
#### 7.1、CRUD 简单案例：
组件 CPage 封装了查询、左侧树形筛选、列表、树形列表、新增、修改、详情、删除、导入、导出、启用、禁用等功能，通过配置选择使用，CPage 分为四块区域，查询区、工具栏区、表格区、新增\修改\详情弹窗区，每块区域对应独立配置，可以很快速很简单实现一个增删查改功能，接口地址、权限默认根据当前路由生成，可以不用设置
...
async function getXxxList (type) {
  if (type) {
...
...
{
  ...
  required: true,
  props: {
    remote: { // 所有组件支持远程数据源配置
      url: '/api/xxx/select',
      params: {
        type: '{formData.type:required}' // 使用'{}'，自动监听表单的 type 字段，变化时，重新获取数据, 若添加:required 表示type有值时才获取数据
      },
      ...
    },
